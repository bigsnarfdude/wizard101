#!/usr/bin/env python3
"""
Batch-Mode Cascade Evaluation

Processes samples layer-by-layer with model unloading between stages.
Memory efficient - only one model loaded at a time.

Usage:
    python evaluate_cascade_batch.py [--samples N] [--seed S]
"""

import gc
import json
import time
import random
import argparse
import torch
from pathlib import Path
from collections import Counter


def clear_gpu():
    """Clear GPU memory."""
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()


def run_l0_batch(samples, threshold=0.7):
    """
    Run L0 on all samples.

    Returns:
        caught: list of (idx, result) for confident decisions
        uncertain: list of (idx, sample) for escalation
    """
    print("\n" + "="*60)
    print("STAGE 1: L0 BOUNCER")
    print("="*60)

    from l0_bouncer import L0Bouncer
    l0 = L0Bouncer()

    caught = []
    uncertain = []

    for idx, sample in enumerate(samples):
        text = sample.get("prompt", sample.get("text", ""))
        result = l0.classify(text)

        if result["confidence"] >= threshold:
            # Confident - stop here
            caught.append((idx, {
                "text": text,
                "expected": sample.get("label", ""),
                "final_label": result["label"],
                "stopped_at": "L0",
                "confidence": result["confidence"],
                "layers": [{"level": "L0", "label": result["label"], "confidence": result["confidence"]}]
            }))
        else:
            # Uncertain - escalate
            uncertain.append((idx, sample))

        if (idx + 1) % 50 == 0:
            print(f"  L0 processed {idx + 1}/{len(samples)}")

    print(f"\nL0 Results:")
    print(f"  Caught: {len(caught)} ({len(caught)/len(samples)*100:.1f}%)")
    print(f"  Uncertain: {len(uncertain)} ({len(uncertain)/len(samples)*100:.1f}%)")

    # Unload
    del l0
    clear_gpu()
    print("  L0 unloaded")

    return caught, uncertain


def run_l1_batch(uncertain_samples, threshold=0.7):
    """
    Run L1 on uncertain samples from L0.

    Returns:
        caught: list of (idx, result) for confident decisions
        uncertain: list of (idx, sample, l1_result) for escalation
    """
    if not uncertain_samples:
        return [], []

    print("\n" + "="*60)
    print("STAGE 2: L1 ANALYST")
    print("="*60)

    from l1_analyst import L1Analyst
    l1 = L1Analyst()

    caught = []
    uncertain = []

    for idx, sample in uncertain_samples:
        text = sample.get("prompt", sample.get("text", ""))
        result = l1.analyze(text)

        if result["confidence"] >= threshold:
            # Confident - stop here
            caught.append((idx, {
                "text": text,
                "expected": sample.get("label", ""),
                "final_label": result["label"],
                "stopped_at": "L1",
                "confidence": result["confidence"],
                "layers": [
                    {"level": "L0", "label": "uncertain", "confidence": 0.5},
                    {"level": "L1", "label": result["label"], "confidence": result["confidence"]}
                ],
                "reasoning": result["reasoning"]
            }))
        else:
            # Uncertain - escalate
            uncertain.append((idx, sample, result))

        if (len(caught) + len(uncertain)) % 20 == 0:
            print(f"  L1 processed {len(caught) + len(uncertain)}/{len(uncertain_samples)}")

    print(f"\nL1 Results:")
    print(f"  Caught: {len(caught)} ({len(caught)/len(uncertain_samples)*100:.1f}%)")
    print(f"  Uncertain: {len(uncertain)} ({len(uncertain)/len(uncertain_samples)*100:.1f}%)")

    # Unload
    del l1
    clear_gpu()
    print("  L1 unloaded")

    return caught, uncertain


def run_l2_batch(uncertain_samples):
    """
    Run L2 on uncertain samples from L1.

    Returns:
        results: list of (idx, result) - L2 makes final decision for remaining
    """
    if not uncertain_samples:
        return []

    print("\n" + "="*60)
    print("STAGE 3: L2 GAUNTLET")
    print("="*60)

    from l2_gauntlet import L2Gauntlet
    l2 = L2Gauntlet()

    results = []

    for idx, sample, l1_result in uncertain_samples:
        text = sample.get("prompt", sample.get("text", ""))
        result = l2.analyze(text)

        results.append((idx, {
            "text": text,
            "expected": sample.get("label", ""),
            "final_label": result["label"],
            "stopped_at": "L2",
            "confidence": 0.9 if result["consensus"] else 0.7,
            "layers": [
                {"level": "L0", "label": "uncertain", "confidence": 0.5},
                {"level": "L1", "label": l1_result["label"], "confidence": l1_result["confidence"]},
                {"level": "L2", "label": result["label"], "votes": result["votes"], "consensus": result["consensus"]}
            ]
        }))

        if len(results) % 10 == 0:
            print(f"  L2 processed {len(results)}/{len(uncertain_samples)}")

    print(f"\nL2 Results: {len(results)} samples processed")

    # L2 uses Ollama, no GPU to clear
    del l2

    return results


def main():
    parser = argparse.ArgumentParser(description="Batch-mode cascade evaluation")
    parser.add_argument("--samples", type=int, default=100, help="Number of samples")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--l0-threshold", type=float, default=0.7, help="L0 confidence threshold")
    parser.add_argument("--l1-threshold", type=float, default=0.7, help="L1 confidence threshold")
    args = parser.parse_args()

    # Load test data
    test_path = Path("guardreasoner_test_10k.json")
    if not test_path.exists():
        test_path = Path("../wildguard_full_benchmark.json")
    if not test_path.exists():
        print("Test data not found")
        return

    print(f"Using test data: {test_path}")

    with open(test_path) as f:
        test_data = json.load(f)

    # Sample
    random.seed(args.seed)
    if len(test_data) > args.samples:
        test_data = random.sample(test_data, args.samples)

    # Normalize labels
    for item in test_data:
        label = item.get("label", "").lower()
        if label == "harmless":
            item["label"] = "safe"

    print(f"\nEvaluating {len(test_data)} samples through batch cascade")
    print(f"L0 threshold: {args.l0_threshold}")
    print(f"L1 threshold: {args.l1_threshold}")

    start_time = time.time()

    # Stage 1: L0
    l0_caught, l0_uncertain = run_l0_batch(test_data, args.l0_threshold)

    # Stage 2: L1
    l1_caught, l1_uncertain = run_l1_batch(l0_uncertain, args.l1_threshold)

    # Stage 3: L2
    l2_results = run_l2_batch(l1_uncertain)

    total_time = time.time() - start_time

    # Aggregate results
    all_results = {}
    for idx, result in l0_caught:
        all_results[idx] = result
    for idx, result in l1_caught:
        all_results[idx] = result
    for idx, result in l2_results:
        all_results[idx] = result

    # Sort by original index
    results = [all_results[i] for i in sorted(all_results.keys())]

    # Calculate stats
    print("\n" + "="*60)
    print("BATCH CASCADE RESULTS")
    print("="*60)

    # Layer distribution
    layer_counts = Counter(r["stopped_at"] for r in results)
    print("\nLayer Distribution:")
    for layer in ["L0", "L1", "L2", "L3"]:
        count = layer_counts.get(layer, 0)
        pct = count / len(results) * 100 if results else 0
        print(f"  {layer}: {count} ({pct:.1f}%)")

    # Accuracy
    correct = sum(1 for r in results if r["final_label"] == r["expected"])
    accuracy = correct / len(results) * 100 if results else 0
    print(f"\nOverall Accuracy: {correct}/{len(results)} = {accuracy:.1f}%")

    # Confusion matrix
    tp = sum(1 for r in results if r["final_label"] == "harmful" and r["expected"] == "harmful")
    tn = sum(1 for r in results if r["final_label"] == "safe" and r["expected"] == "safe")
    fp = sum(1 for r in results if r["final_label"] == "harmful" and r["expected"] == "safe")
    fn = sum(1 for r in results if r["final_label"] == "safe" and r["expected"] == "harmful")

    print("\nConfusion Matrix:")
    print(f"  TP: {tp}, TN: {tn}, FP: {fp}, FN: {fn}")

    # Metrics
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0

    print(f"\nMetrics:")
    print(f"  Precision: {precision*100:.1f}%")
    print(f"  Recall: {recall*100:.1f}%")
    print(f"  F1: {f1*100:.1f}%")

    print(f"\nTotal Time: {total_time:.1f}s ({total_time/len(results)*1000:.1f}ms/sample)")

    # Save results
    output_path = Path("cascade_batch_results.json")
    with open(output_path, "w") as f:
        json.dump({
            "config": {
                "samples": len(results),
                "seed": args.seed,
                "l0_threshold": args.l0_threshold,
                "l1_threshold": args.l1_threshold,
            },
            "summary": {
                "accuracy": accuracy,
                "precision": precision * 100,
                "recall": recall * 100,
                "f1": f1 * 100,
                "total_time_s": total_time,
                "layer_distribution": dict(layer_counts),
            },
            "confusion_matrix": {"tp": tp, "tn": tn, "fp": fp, "fn": fn},
            "results": results,
        }, f, indent=2)

    print(f"\nResults saved to {output_path}")

    # Show some examples
    print("\n" + "="*60)
    print("EXAMPLE RESULTS")
    print("="*60)

    incorrect = [r for r in results if r["final_label"] != r["expected"]][:3]
    for r in incorrect:
        text = r["text"][:60] + "..." if len(r["text"]) > 60 else r["text"]
        print(f"\n✗ {text}")
        print(f"  Expected: {r['expected']}, Got: {r['final_label']} @ {r['stopped_at']}")


if __name__ == "__main__":
    main()
