#!/usr/bin/env python3
"""
Evaluate GuardReasoner-8B with 4-bit quantization (FAST!)
Uses bitsandbytes for efficient 4-bit inference
"""
...
# Switch to 8B model with 4-bit quantization
...
def load_model():
    """Load GuardReasoner model with 4-bit quantization"""
    print("="*70)
    print("LOADING GUARDREASONER-8B (4-BIT QUANTIZED)")
    print("="*70)
    print(f"Model: {MODEL_ID}")
    print(f"Quantization: 4-bit (2-3x faster!)")
    print()
...
def main():
    print("="*70)
    print("GUARDREASONER EVALUATION - 4-BIT QUANTIZED (FAST!)")
    print("="*70)
    ...
    print("⚡ 4-bit quantization = 2-3x faster than float16!")
    print()
    ...
    print(f"Our F1 (4-bit): {metrics['f1']:.3f}")
    print(f"Difference: {metrics['f1'] - 0.840:+.3f}")
    print("="*70)

    # Save results
    output = BASE_DIR / "guardreasoner/results_4bit_quantized.json"
