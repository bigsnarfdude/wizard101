#!/usr/bin/env python3
"""Experiment 06: Safeguard model + Minimal policies"""
...
# Override load_policies to use policies_minimal
def load_policies_06():
    policies = {}
    import os
    for filename in os.listdir('policies_minimal'):
        if filename.endswith('.txt'):
            policy_name = filename.replace('.txt', '')
            with open(os.path.join('policies_minimal', filename)) as f:
                policies[policy_name] = f.read()
    return policies

serial_gauntlet_simple.load_policies = load_policies_06
...
if __name__ == '__main__':
    ...
    sys.argv = ['experiment_06.py', 'wildguard_benchmark.json']
    run_eval()
    ...
    print(f"Policies: policies_minimal")
